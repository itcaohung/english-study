(function () {
  ...
  function avatarOptions(selected) {
    return `<fieldset class="avatar-options"><legend>Chọn bạn đồng hành</legend>${S.avatars.map((a, i) => `<label><input type="radio" name="learner-avatar" value="${a}" ${a === selected ? 'checked' : ''}><span role="img" aria-label="${['Cú mèo', 'Cáo', 'Gấu trúc', 'Mèo', 'Cá heo', 'Thỏ', 'Hổ', 'Koala'][i]}">${a}</span></label>`).join('')}<label class="avatar-upload"><input type="file" id="avatar-upload" accept="image/png,image/jpeg,image/webp" aria-label="Tải ảnh làm avatar"><span id="avatar-upload-preview">${isCustomAvatar(selected) ? avatarContent(selected, 'Ảnh avatar hiện tại') : '＋'}</span><small>Ảnh</small></label></fieldset><p class="avatar-help">Hoặc chọn ảnh PNG, JPG hay WebP. Ảnh được cắt vuông, nén và chỉ lưu trong hồ sơ này.</p>`;
  }
  function prepareAvatar(file) {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        image.onload = () => {
          const canvas = document.createElement('canvas'), size = 160, side = Math.min(image.naturalWidth, image.naturalHeight);
